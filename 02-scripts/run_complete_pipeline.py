...
finnhub_module = importlib.import_module('00b_finnhub_news')
main_analysis_module = importlib.import_module('05_main_analysis')

DataAcquisition = data_acq_module.DataAcquisition
FinnhubNewsAcquisition = finnhub_module.FinnhubNewsAcquisition
...
class CompletePipeline:
    ...
    def __init__(self,
        ...
        finnhub_api_key : str, optional
            Finnhub API key (will try .env if not provided)
        data_dir : str
            Data directory
        output_dir : str
            Output directory
    ...
    def _stage1_data_acquisition(self):
        ...
        # 1.2: Finnhub news data
        print("\n[1.2] Downloading news data from Finnhub...")
        print("-" * 80)

        news_acquirer = FinnhubNewsAcquisition(
            api_key=self.finnhub_api_key,
            output_dir=self.data_dir
        )

        news_data = news_acquirer.download_all_news_data(
            symbol=self.ticker,
            start_date=self.start_date,
            end_date=self.end_date,
...
def run_pipeline(ticker: str,
    ...
    finnhub_api_key : str, optional
        Finnhub API key (loads from .env if not provided)
    skip_data_download : bool
    ...
    run_pipeline('AAPL', '2020-01-01', skip_data_download=True)
    """
