"""
MULTI-SECTOR NEWS DATA ACQUISITION
===================================

Downloads news data for 6 major stocks from EODHD API.
"""
...
# Configuration
# Note: NVDA, JPM, PFE, XOM already downloaded
TICKERS = ['AMZN', 'BA']
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"    Batch {offset // batch_size + 1}: {len(data)} articles")
                ...
                print(f"    ⚠ Payment required - upgrade plan for historical data")
                break
            else:
...
def save_news_data(df: pd.DataFrame, ticker: str):
...
def main():
    """Download news for all stocks"""
    print("=" * 80)
    print("MULTI-SECTOR NEWS DATA ACQUISITION")
    print("=" * 80)
    ...
    for i, ticker in enumerate(TICKERS, 1):
        ...
        if df is not None and not df.empty:
            if save_news_data(df, ticker):
                ...
                stats.append({
                    ...
                    'Has_Sentiment': 'sentiment_polarity' in df.columns
                })
            else:
        ...
        # Rate limiting
        if i < len(TICKERS):
            print("  Waiting 2 seconds (API rate limit)...")
            time.sleep(2)
    ...
    print("=" * 80)
    print(f"✓ Successful: {len(results['success'])}/{len(TICKERS)}")
    if results['success']:
    ...
    if stats:
        ...
        stats_df.to_csv(DATA_DIR / 'news_download_summary.csv', index=False)
        print(f"\n✓ Saved summary to {DATA_DIR / 'news_download_summary.csv'}")
